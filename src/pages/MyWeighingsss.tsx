...
export function MyWeighings() {
  ...
  async function fecthPlants() {
    ...
    const data = await loadWeighings();
    if (!data) return setLoading(true);

    if (page > 1) {
      setWeighings((oldValue) => [...oldValue, ...data]);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Você ainda não tem pesagens.</Text>
        <Text style={styles.subtitle}>
          Clique em Nova pesagem para começar.
        </Text>
      </View>
      ...
      <View style={styles.plants}>
        {/* <FlatList data={weighings} keyExtractor={(item) => String(item.id)}
                    renderItem={({ item }) => (
